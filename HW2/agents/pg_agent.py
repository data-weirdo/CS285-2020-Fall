...
class PGAgent(BaseAgent):
    ...
    def train(self, observations, actions, rewards_list, next_observations, terminals):
        ...
        # TODO: step 3: use all datapoints (s_t, a_t, q_t, adv_t) to update the PG actor/
    ...
    def _discounted_return(self, rewards):
        ...
        power = np.arange(0, timesteps)
        gamma = self.gamma ** power
        discounted_returns = np.multiply(gamma, rewards)
        list_of_discounted_returns = np.repeat(discounted_returns, timesteps)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for i in range(timesteps):
            list_of_discounted_cumsums.append(np.multiply(gamma[:timesteps-i], rewards[i:]))

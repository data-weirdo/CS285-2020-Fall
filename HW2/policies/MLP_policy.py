...
class MLPPolicy(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        action = self.forward(obs).sample().numpy()
    ...
    def forward(self, observation: torch.FloatTensor):
        ...
        if self.discrete:
            ...
            action_distribution = distributions.MultivariateNormal(loc=self.mean_net(observation),
                                                                   covariance_matrix=torch.diag(self.logstd))
...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        if self.nn_baseline:
            ...
            targets = normalize(q_values, torch.mean(q_values), torch.std(q_values))
